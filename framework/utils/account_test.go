...
import (
	...
	utilsecp256k1 "github.com/stratosnet/framework/utils/crypto/secp256k1"
...
func TestCreateWallet(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		...
		addr, err := CreateWallet("keys", "", password, txclienttypes.StratosBech32Prefix, mnemonic, "", "m/44'/606'/0'/0/0")
		if err != nil {
			t.Fatal(err.Error())
		}
		...
		bechPub, err := bech32.ConvertAndEncode(txclienttypes.AccountPubKeyPrefix, sdkPubkey.Bytes())
		if err != nil {
			t.Fatal(err.Error())
		}
...
func TestCreateP2PKey(t *testing.T) {
	...
	for i := 0; i < 1; i++ {
		...
		addr, err := CreateP2PKey("keys", "", password, txclienttypes.SdsNodeP2PAddressPrefix)
		if err != nil {
			t.Fatal(err.Error())
		}
		...
		bechPub, err := bech32.ConvertAndEncode(txclienttypes.SdsNodeP2PPubkeyPrefix, pubKey.Bytes())
		if err != nil {
			t.Fatal(err.Error())
		}
...
func TestDecryptPrivValidatorKeyJson(t *testing.T) {
	...
	pubKeyJson, err := json.Marshal(&sdkPubKey)
	if err != nil {
		t.Fatal(err.Error())
	}
