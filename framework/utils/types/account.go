...
func WalletAddressFromBech(str string) (Address, error) {
	addr, err := types.GetFromBech32(str, types.AccountAddressPrefix)
	if err != nil {
		return Address{}, err
	}
...
func P2pAddressFromBech(str string) (Address, error) {
	...
	bz, err := types.GetFromBech32(str, types.SdsNodeP2PAddressPrefix)
	if err != nil {
		return Address{}, err
	}
...
func WalletPubkeyFromBech(str string) (AccPubKey, error) {
	pubkey, err := types.GetFromBech32(str, types.AccountPubKeyPrefix)
	if err != nil {
		fmt.Println(err)
		return AccPubKey{}, err
	}
	err = types.VerifyAddressFormat(pubkey)
	if err != nil {
		return AccPubKey{}, err
	}
...
func (p AccPrivKey) PubKeyFromPrivKey() AccPubKey {
	return BytesToAccPubKey(secp256k1.Generate(p.Bytes()).PubKey().Bytes())
}
...
func P2pPubKeyFromBech(str string) (P2pPubKey, error) {
	pubkey, err := types.GetFromBech32(str, types.SdsNodeP2PPubkeyPrefix)
	if err != nil {
