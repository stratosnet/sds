...
then
  ...
  sed -i "s!stratos_chain_url = '.*'!stratos_chain_url = '$STCHAIN_URL'!g" $WORK_DIR/config/config.toml

  echo "[entrypoint] Set chain_id as '$CHAIN_ID'"
  sed -i "s!chain_id = '.*'!chain_id = '$CHAIN_ID'!g" $WORK_DIR/config/config.toml

  echo "[entrypoint] Set the default SDS meta node as '$SP_LIST'"
  sed -i "/\[\[sp_list\]\]/,/^network_address = '.*'$/ s/.*//" $WORK_DIR/config/config.toml
  for sp in $SP_LIST
  do
    cat << EOF >> $WORK_DIR/config/config.toml
[[sp_list]]
p2p_address = ''
p2p_public_key = ''
network_address = '$sp'
EOF
  ...
  sed -i "s/rest_port = '.*'/rest_port= '9608'/g" $WORK_DIR/config/config.toml
  sed -i "s/internal_port = '.*'/internal_port = '9708'/g" $WORK_DIR/config/config.toml
  sed -i "s/rpc_port = '.*'/rpc_port = '8135'/g" $WORK_DIR/config/config.toml
  sed -i "s/metrics_port = '.*'/metrics_port = '8765'/g" $WORK_DIR/config/config.toml
  sed -i "s/metrics_port = '.*'/metrics_port = '8765'/g" $WORK_DIR/config/config.toml
  sed -i "/\[monitor\]/,/^port = '.*'$/ s/^port = '.*'$/port = '5433'/" $WORK_DIR/config/config.toml
fi
