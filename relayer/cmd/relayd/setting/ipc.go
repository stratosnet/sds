...
func DefaultIPCEndpoint(homePath string) string {
	return IPCEndpoint(DefaultDataDir(homePath), "relayer.ipc")
}
...
func DefaultDataDir(homePath string) string {
	...
	if home != "" {
		switch runtime.GOOS {
		case "darwin":
			return filepath.Join(home, "Library", "Stratos_relay")
		case "windows":
			// We used to put everything in %HOME%\AppData\Roaming, but this caused
			// problems with non-typical setups. If this fallback location exists and
			// is non-empty, use it, otherwise DTRT and check %LOCALAPPDATA%.
			fallback := filepath.Join(home, "AppData", "Roaming", "Stratos_relay")
