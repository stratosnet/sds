...
func NewInt64DecCoin(denom string, amount int64) DecCoin {
	return NewDecCoin(denom, sdktypes.NewInt(amount))
}
...
func (coins DecCoins) Intersect(coinsB DecCoins) DecCoins {
	...
	for i, coin := range coins {
		minCoin := DecCoin{
			Denom:  coin.Denom,
			Amount: sdktypes.MinDec(coin.Amount, coinsB.AmountOf(coin.Denom)),
		}
		res[i] = minCoin
	}
...
func ParseDecCoin(coinStr string) (coin DecCoin, err error) {
	...
	amount, err := sdktypes.NewDecFromStr(amountStr)
	if err != nil {
