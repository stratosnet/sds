...
tests:
	for i, test := range tests {
		...
		var version []byte
		if !strings.HasSuffix(test.name, "testnet") {
			version = chaincfg.MainNetParams.HDPrivateKeyID[:]
		} else {
			version = chaincfg.TestNet3Params.HDPrivateKeyID[:]
		}

		extKey := MasterKeyGenerate(masterSeed, Secp256k1CurvePhrase)
		//if err != nil {
		...
		for _, childNum := range test.path {
			var err error
			extKey, err = gen.Child(*extKey, childNum)
			if err != nil {
				t.Errorf("err: %v", err)
				continue tests
			}
		...
		key := hdkeychain.NewExtendedKey(version, extKey.key, extKey.chainCode, extKey.parentFP, extKey.depth, extKey.childNum, extKey.isPrivate)
		if key.Depth() != uint8(len(test.path)) {
			t.Errorf("Depth of key %d should match fixture path: %v",
				key.Depth(), len(test.path))
			continue
		}
