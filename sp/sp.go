...
func StartSP(conf string) *net.Server {
	...
	spbf.Register(header.ReqFindMyAlbum, events.GetFindMyAlbumHandler(server))                         // server.ListenEvent(header.ReqFindMyAlbum, new(events.findMyAlbum))
	spbf.Register(header.ReqSearchAlbum, events.GetSearchAlbumHandler(server))                         // server.ListenEvent(header.ReqSearchAlbum, new(events.searchAlbum))
	spbf.Register(header.ReqInvite, events.GetInviteHandler(server))                                   // server.ListenEvent(header.ReqInvite, new(events.invite))
	spbf.Register(header.ReqGetReward, events.GetGetRewardHandler(server))                             // server.ListenEvent(header.ReqGetReward, new(events.getReward))
	spbf.Register(header.ReqCollectionAlbum, events.GetCollectAlbumHandler(server))                    // server.ListenEvent(header.ReqCollectionAlbum, new(events.collectAlbum))
