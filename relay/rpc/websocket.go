...
func (s *Server) WebsocketHandler(allowedOrigins []string, ctx context.Context) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			utils.DebugLog("WebSocket upgrade failed", "err", err)
			return
		}
		codec := newWebsocketCodec(conn)
		s.ServeCodecWithContext(codec, ctx)
	})
}
...
func wsHandshakeValidator(allowedOrigins []string) func(*http.Request) bool {
	...
	utils.DebugLog(fmt.Sprintf("Allowed origin(s) for WS RPC interface %v", origins.ToSlice()))
	...
	f := func(req *http.Request) bool {
		...
		utils.WarnLog("Rejected WebSocket connection", "origin", origin)
		return false
	}
...
func ruleAllowsOrigin(allowedOrigin string, browserOrigin string) bool {
	...
	if err != nil {
		utils.WarnLog("Error parsing allowed origin specification", "spec", allowedOrigin, "error", err)
		return false
	}
	browserScheme, browserHostname, browserPort, err = parseOriginURL(browserOrigin)
	if err != nil {
		utils.WarnLog("Error parsing browser 'Origin' field", "Origin", browserOrigin, "error", err)
		return false
	}
