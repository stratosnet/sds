...
func DecryptKey(keyjson []byte, auth string) (*AccountKey, error) {
	...
	privKey := secp256k1.Generate(hdKeyBytesObject.PrivKey)

	return &AccountKey{
		Id:         uuid.UUID(keyId),
		Name:       k.Name,
		Address:    txclienttypes.AccAddress(privKey.PubKey().Address()), //secp256k1.PrivKeyToAddress(hdKeyBytesObject.PrivKey),
